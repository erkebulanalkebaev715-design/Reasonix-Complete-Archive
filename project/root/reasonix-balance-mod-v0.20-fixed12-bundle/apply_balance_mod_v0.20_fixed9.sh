...
grep -q 'const balanceModVersion = "balance-mod-v0.20"' internal/serve/mod_bridge.go || { echo 'ERROR: Fixed9 requires installed balance-mod-v0.20' >&2; exit 3; }
...
PATHS=(
  ...
  docs/BALANCE_MOD_V020_FIXED9.md
  ...
  internal/agent/balance_strict_retry_fixed9_test.go
  internal/provider/balance_strict_retry_fixed9_test.go
)
...
for p in internal/agent/balance_strict_retry_fixed*_test.go internal/provider/balance_strict_retry_fixed*_test.go; do
  case "$p" in
    internal/agent/balance_strict_retry_fixed9_test.go|internal/provider/balance_strict_retry_fixed9_test.go) ;;
    *) STALE_TESTS+=("$p") ;;
  esac
done
shopt -u nullglob
...
TMP="$(mktemp -d)"
PATCH="$TMP/v020-fixed9.patch"
BACKUP="$TARGET/.balance_mod_backups/v0.20-fixed9-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP"
APPLIED=0
VALIDATED=0
...
rollback_on_error() {
  ...
  if [[ $rc -ne 0 && $APPLIED -eq 1 && $VALIDATED -eq 0 ]]; then
    echo "Fixed9 local validation failed; restoring pre-install files..." >&2
...
new="""// strict-one-paid-attempt-v0.20-fixed9: strict hard-budget mode allows exactly
...
if pat.search(s):
    ...
    for m in ('strict-one-paid-attempt-v0.20-fixed5','strict-one-paid-attempt-v0.20-fixed6','strict-one-paid-attempt-v0.20-fixed7'):
        s=s.replace(m,'strict-one-paid-attempt-v0.20-fixed9')
elif 'strict-one-paid-attempt-v0.20-fixed9' not in s:
...
if 'strictPreCallRetryFactor' in s: raise SystemExit('ERROR: obsolete strictPreCallRetryFactor survived Fixed9')
if 'strict-one-paid-attempt-v0.20-fixed9' not in s: raise SystemExit('ERROR: Fixed9 run_budget marker missing')
p.write_text(s)
...
\t\t// strict-one-paid-attempt-v0.20-fixed9: no hidden connection/header retry.
...
if 'strict-one-paid-attempt-v0.20-fixed9' not in s:
    if any(m in s for m in ('strict-one-paid-attempt-v0.20-fixed5','strict-one-paid-attempt-v0.20-fixed6','strict-one-paid-attempt-v0.20-fixed7')):
        ...
        repl=') (streamedTurn, bool) {\n\tif a.StrictPreCallBudget() {\n\t\t// strict-one-paid-attempt-v0.20-fixed9: fallback would be a second paid call.\n\t\treturn streamedTurn{}, false\n\t}\n\tif !a.activateMissingReasoningFallback() {'
...
if 'strict-one-paid-attempt-v0.20-fixed9' not in s:
    if any(m in s for m in ('strict-one-paid-attempt-v0.20-fixed5','strict-one-paid-attempt-v0.20-fixed6','strict-one-paid-attempt-v0.20-fixed7')):
        ...
        injected=m.group(1)+'''\t\tif a.StrictPreCallBudget() {\n\t\t\t// strict-one-paid-attempt-v0.20-fixed9: replay is another paid request.\n\t\t\tshouldRetry = false\n\t\t}\n'''+m.group(2)
...
if [[ -s "$PATCH" ]]; then
  git apply --check "$PATCH" || { echo 'ERROR: Fixed9 exact apply check failed; refusing fuzzy apply' >&2; exit 4; }
  echo 'v0.20 Fixed9 exact apply check: PASS'
  git apply "$PATCH"
  APPLIED=1
else
  echo 'v0.20 Fixed9 already exact: PASS'
fi

if (( ${#STALE_TESTS[@]} > 0 )); then
  APPLIED=1
  echo "v0.20 Fixed9 stale generated-test cleanup: ${#STALE_TESTS[@]} file(s)"
  rm -f -- "${STALE_TESTS[@]}"
fi
...
if find internal/agent internal/provider -maxdepth 1 -type f \
    ...
    ! -name 'balance_strict_retry_fixed9_test.go' -print -quit | grep -q .; then
  echo 'ERROR: stale balance strict-retry test survived cleanup' >&2
  exit 5
fi

"$GOFMT_BIN" -w "${CORE_PATHS[@]}" internal/agent/balance_strict_retry_fixed9_test.go internal/provider/balance_strict_retry_fixed9_test.go
...
echo 'v0.20 Fixed9 targeted tests...'
...
echo 'v0.20 Fixed9 package compile-only gate...'
# Compile every test file in the touched packages but intentionally execute no
# unrelated runtime/stress tests. Full internal/agent regression contains
# session write-authority/lease probes that are environment-sensitive under
# Android PRoot and are not part of this installer patch. Targeted tests above
# execute the changed budget/retry behavior; this gate catches package/test
# compile errors, duplicate declarations, and link/type regressions.
PATH="$(dirname "$GO_BIN"):$PATH" GOTOOLCHAIN=local "$GO_BIN" test ./internal/provider ./internal/agent ./internal/serve -run '^$' -count=1
PATH="$(dirname "$GO_BIN"):$PATH" GOTOOLCHAIN=local CGO_ENABLED=0 "$GO_BIN" build -o "$TMP/reasonix-fixed9" ./cmd/reasonix
[[ -x "$TMP/reasonix-fixed9" ]] || { echo 'ERROR: Fixed9 temporary CLI build missing' >&2; exit 5; }

grep -q 'strict-one-paid-attempt-v0.20-fixed9' internal/agent/run_budget.go || { echo 'ERROR: Fixed9 marker missing after validation' >&2; exit 5; }
...
mkdir -p bin
BIN_TMP="bin/.reasonix-fixed9.$$"
cp "$TMP/reasonix-fixed9" "$BIN_TMP"
chmod 0755 "$BIN_TMP"
mv -f "$BIN_TMP" bin/reasonix
VALIDATED=1

echo 'v0.20 Fixed9 local validation: PASS'
echo '  targeted changed-behavior tests: PASS'
echo '  touched packages compile-only: PASS'
