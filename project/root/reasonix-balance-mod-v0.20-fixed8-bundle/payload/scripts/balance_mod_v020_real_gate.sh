...
grep -q 'strict-one-paid-attempt-v0.20-fixed8' internal/agent/run_budget.go || fail 'Fixed8 core marker missing; refusing paid API call'
...
echo '  core marker=Fixed8 | obsolete 66-way reserve=ABSENT | provider retries=0 | body replay=off | reasoning replay=off'
...
echo '[REAL 12/14] wait for provider result; fail immediately on typed/runtime error'
PASS=0
LAST_SPENT='0'
...
def walk(x):
    if isinstance(x,dict):
        ...
        text=' '.join(str(x.get(k,'')) for k in ('message','reason','error','detail')).lower()
        strict_bad=('pre-call cost budget','cannot be enforced without model pricing','cannot safely admit prompt','strict pre-call')
        if any(b in typ for b in bad) or any(b in text for b in strict_bad):
            msg=' | '.join(str(x.get(k,'')) for k in ('type','event','kind','phase','status','message','reason','error','detail') if x.get(k))
