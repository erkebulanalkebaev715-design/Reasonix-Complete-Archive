...
echo '[REAL 1/13] validate approval + API key + FX + hard limits'
...
echo '[REAL 2/13] build current Reasonix CLI'
mkdir -p bin
...
echo '[REAL 3/13] create isolated Reasonix runtime + credential store'
TMP="$(mktemp -d)"
PID=''
SERVER_PID=''
SSE_PID=''
...
echo '[REAL 4/13] zero-generation DeepSeek API preflight (/models + /user/balance)'
set +e
...
echo '[REAL 5/13] verify strict pre-call pricing is known BEFORE Provider.Stream'
...
echo '[REAL 6/13] verify Reasonix sees the same provider credential'
set +e
(
  cd "$TMP"
...
echo '[REAL 7/13] start token-authenticated localhost backend'
(
  cd "$TMP"
  HOME="$TMP/home" REASONIX_HOME="$TMP/rxhome" \
...
echo '[REAL 8/13] apply and verify hard pre-call KZT budget'
...
echo '[REAL 9/13] open authenticated SSE telemetry'
set +e
...
echo '[REAL 10/13] submit EXACTLY ONE DeepSeek V4 Flash model task'
...
echo '[REAL 11/13] wait for provider result; fail immediately on typed/runtime error'
PASS=0
LAST_SPENT='0'
...
echo '[REAL 12/13] verify Flash-only path + reconcile provider-reported usage'
"$CURL_BIN" -fsS --connect-timeout 5 --max-time 10 \
...
echo '[REAL 13/13] final hard-budget assertion'
