...
grep -q 'const balanceModVersion = "balance-mod-v0.19"' internal/serve/mod_bridge.go
...
for _ in $(seq 1 160); do
  if [[ -s "$PORT_FILE" && -s "$TOKEN_FILE" && -s "$PID_FILE" ]]; then break; fi
...
assert p['modVersion']=='balance-mod-v0.19', p
...
set +e
"$CURL_BIN" -sS -N --max-time 1 -D "$TMP/events.headers" -o "$TMP/events.body" \
  -b "$COOKIE_JAR" "$BASE/mod/events"
SSE_RC=$?
set -e
...
grep -qi '^content-type:.*text/event-stream' "$TMP/events.headers" || {
  ...
  echo "BALANCE_V19_FAIL: /mod/events is not authenticated SSE" >&2
  exit 1
}
