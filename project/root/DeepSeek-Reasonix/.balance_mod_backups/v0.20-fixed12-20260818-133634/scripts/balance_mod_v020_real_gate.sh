...
HARNESS_REV='v0.20-fixed10-semantic-completion'
...
save_sanitized_snapshot() {
  ...
  for f in models.json balance-api.json doctor.json doctor.err status.json before.json start.json live.json poll-budget.json after.json events.err events.sse; do
    if [[ -s "$TMP/$f" ]]; then
