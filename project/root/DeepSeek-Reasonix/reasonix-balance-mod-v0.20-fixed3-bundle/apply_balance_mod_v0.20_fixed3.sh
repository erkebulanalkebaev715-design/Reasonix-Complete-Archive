...
# Stop only stale processes that belong to an interrupted v0.20 real gate.
STOPPED="$($PY_BIN - <<'PY'
import os,signal
...
TMP="$(mktemp -d)"
PATCH="$TMP/v020-fixed3.patch"
BACKUP="$TARGET/.balance_mod_backups/v0.20-fixed3-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP"
: > "$PATCH"
trap 'rm -rf "$TMP"' EXIT
...
if [[ -s "$PATCH" ]]; then
  git apply --check "$PATCH" || { echo 'ERROR: v0.20 Fixed3 exact apply check failed; refusing fuzzy apply' >&2; exit 4; }
  echo 'v0.20 Fixed3 patch apply check: PASS'
  git apply "$PATCH"
else
  echo 'v0.20 Fixed3 already exact'
fi
...
if [[ -s "$PATCH" ]]; then
  git apply --reverse --check "$PATCH" || { echo 'ERROR: v0.20 Fixed3 reverse-apply check failed' >&2; exit 6; }
  echo 'v0.20 Fixed3 reverse-apply check: PASS'
fi

echo 'Balance Mod v0.20 Fixed3 applied; diff check: PASS'
echo "Backup of replaced v0.20 files: $BACKUP"
echo 'No Go test/build, API-key read, or provider call was performed by this installer.'
