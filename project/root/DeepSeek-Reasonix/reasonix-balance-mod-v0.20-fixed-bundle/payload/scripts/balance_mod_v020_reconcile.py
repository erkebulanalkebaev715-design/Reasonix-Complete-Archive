...
def calc(u,m,fx):
 b=band(dt(u['ratedAt']),m); r=m['pricingSnapshot']['deepseek-v4-flash'][b]; unit=m['pricingSnapshot']['unitTokens']
...
def spent(path):
 ...
 return float(v)
def selftest():
 m={'pricingSnapshot':{'unitTokens':1000000,'peakUtcWindows':[[1,4],[6,10]],'deepseek-v4-flash':{'peak':{'cacheHit':.014,'input':.44,'output':1.32},'offPeak':{'cacheHit':.007,'input':.22,'output':.66}}}}
 u={'prompt':1000,'completion':100,'hit':400,'miss':600,'ratedAt':'2026-08-18T02:00:00Z'}; usd,kzt,b=calc(u,m,500); exp=(400*.014+600*.44+100*1.32)/1e6
 assert b=='peak' and abs(usd-exp)<1e-12 and abs(kzt-exp*500)<1e-12; print('V020_RECONCILE_SELFTEST_PASS')
...
usd,expected,b=calc(u,m,a.fx); delta=spent(a.after)-spent(a.before)
if delta<=0: raise SystemExit(f'V020_RECONCILE_FAIL: spent delta not positive {delta}')
tol=max(float(m['reconciliation']['absoluteToleranceKzt']),expected*float(m['reconciliation']['relativeTolerance']))
if abs(delta-expected)>tol: raise SystemExit(f'V020_RECONCILE_FAIL: expectedKzt={expected:.6f} backendDeltaKzt={delta:.6f} tolerance={tol:.6f} usage={u}')
print(f"V020_RECONCILE_PASS prompt={int(u['prompt'])} hit={int(u['hit'])} miss={int(u['miss'])} output={int(u['completion'])} band={b} expectedUSD={usd:.8f} expectedKZT={expected:.6f} backendDeltaKZT={delta:.6f}")
