...
if [[ -s "$PATCH" ]]; then
  git apply --check "$PATCH" || { echo 'ERROR: v0.20 Fixed exact apply check failed; refusing fuzzy apply' >&2; exit 4; }
  echo 'v0.20 Fixed patch apply check: PASS'
  git apply "$PATCH"
else
  echo 'v0.20 Fixed already exact'
fi
...
if [[ -s "$PATCH" ]]; then
  git apply --reverse --check "$PATCH" || { echo 'ERROR: v0.20 Fixed reverse-apply check failed' >&2; exit 6; }
  echo 'v0.20 Fixed reverse-apply check: PASS'
fi

echo 'Balance Mod v0.20 Fixed applied; diff check: PASS'
