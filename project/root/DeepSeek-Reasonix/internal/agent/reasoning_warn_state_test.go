...
func TestMissingReasoningWarnStateReadFailureDoesNotOverwriteExistingIncidents(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("directory-as-file read failures are not portable to Windows")
	}
	dir := t.TempDir()
	path := filepath.Join(dir, missingReasoningWarnStateFilename)
	s := newMissingReasoningWarnState(dir)
	...
	// A directory at the state path fails every open-for-read deterministically,
	// including for root, unlike chmod 0 which CAP_DAC_OVERRIDE bypasses.
