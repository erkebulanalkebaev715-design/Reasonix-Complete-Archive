...
// is bounded so the whole component stays under the platform filename limit
// (config.FilenameComponentMaxBytes; Android/FUSE rejects 255-byte names).
...
func statsPath(name string) string {
	...
	// Component budget: slug + "-" + 8 hex digits + ".stats.json", kept under
	// config.FilenameComponentMaxBytes so Android/FUSE hosts that reject
	// 255-byte names can still persist plugin telemetry.
	stem := slug(name)
	if maxStem := config.FilenameComponentMaxBytes - len("-00000000.stats.json"); len(stem) > maxStem {
		stem = stem[:maxStem]
	}
