...
func TestStatsPathBoundedForLongNames(t *testing.T) {
	...
	if base := filepath.Base(p); len(base) > config.FilenameComponentMaxBytes {
		t.Fatalf("component = %d bytes, exceeds %d-byte platform limit: %q", len(base), config.FilenameComponentMaxBytes, base)
	}
