...
const (
	...
	ToolResultPreview
	// Swarm events (Reasonix 3.0). Each carries a SwarmEvent payload naming the
	// sub-event (swarm.started, swarm.task.created, ...). Appended last to keep
	// earlier Kind values wire-stable; older clients ignore unknown kinds.
	SwarmStarted
	SwarmTaskCreated
	SwarmTaskAssigned
	SwarmAgentStarted
	SwarmAgentReasoning
	SwarmAgentToolDispatch
	...
	SwarmMergeStarted
	SwarmMergeCompleted
	SwarmVerificationStarted
	SwarmVerificationCompleted
	SwarmCompleted
	SwarmFailed
	SwarmCancelled
...
type Event struct {
	...
	// Swarm is set on Swarm* events (Reasonix 3.0). It carries only structured,
	// content-free orchestration state — never raw transcripts, prompts, or
	// hidden reasoning.
	Swarm *SwarmEvent
}

// SwarmEvent is the content-free payload on Swarm* events (Reasonix 3.0). The
// SubEvent field names the concrete swarm lifecycle event; the rest carries
// structured orchestration state for observability only.
type SwarmEvent struct {
	SwarmID  string
	TaskID   string
	WorkerID string
	SubEvent string // swarm.started | swarm.task.created | ...
	Status   string // task/swarm status at emission
	ModelRef string
	Provider string
	Summary  string
	Failure  string
