...
// SwarmStateDir is the durable root for Reasonix 3.0 swarm state. It stores
// only structured orchestration state (tasks, findings, budgets, results);
// raw transcripts, prompts, and hidden reasoning are never written there.
func SwarmStateDir() string {
	home := reasonixHomeDir()
	if strings.TrimSpace(home) == "" {
		return ""
	}
	return filepath.Join(home, "state", "swarm")
}
...
func WorkspaceSlug(absPath string) string {
	...
	return boundFilenameComponent(slug, FilenameComponentMaxBytes)
}

// FilenameComponentMaxBytes is the safe byte budget for a single derived
// filename component. Common filesystems report NAME_MAX=255, but Android/FUSE
// hosts (PRoot/Termux) reserve one byte and reject 255-byte components, so the
// project bounds derived names one byte lower to stay writable everywhere.
const FilenameComponentMaxBytes = 254

// boundFilenameComponent caps a derived filename component at the common
// per-component filesystem limit (255 bytes on ext4/APFS/NTFS; the project
// passes FilenameComponentMaxBytes to stay safe on Android/FUSE). maxLen is the
// byte budget for this component (path segments pass the limit; names that gain
// an extension pass the limit minus the extension length). Inputs at or under
// the budget pass through byte-identical — every component that ever existed on
...
// component (pass FilenameComponentMaxBytes for a bare path segment; subtract
// the extension length when one will be appended).
func BoundFilenameComponent(s string, maxLen int) string {
