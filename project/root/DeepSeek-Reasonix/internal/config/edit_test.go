...
// TestSaveToUnwritableUserSymlinkTargetPreservesLink proves a save through a
// symlink to an unwritable target fails without replacing the link or mutating
// the target. The target is swapped for a directory because permission bits are
// bypassed by root (CAP_DAC_OVERRIDE); a rename onto an existing directory
// fails deterministically for every user, including root.
func TestSaveToUnwritableUserSymlinkTargetPreservesLink(t *testing.T) {
	...
	original := []byte("default_model = \"old\"\n")
