...
HARNESS_REV='v0.20-fixed12-legacy-compatible-usage-receipt'
...
diagnostics() {
  set +e
  if [[ -n "${TMP:-}" ]]; then
    for f in models.json balance-api.json doctor.json doctor.err status.json before.json start.json live.json poll-budget.json provider-usage.json events.err; do
      if [[ -s "$TMP/$f" ]]; then
        echo "----- redacted $f -----" >&2
...
save_sanitized_snapshot() {
  ...
  local dest="$ROOT/.balance_mod_diagnostics/v020-fixed12-last"
  rm -rf "$dest" 2>/dev/null || true
  mkdir -p "$dest" || return 0
  chmod 700 "$dest" 2>/dev/null || true
  for f in models.json balance-api.json doctor.json doctor.err status.json before.json start.json live.json poll-budget.json after.json provider-usage.json events.err events.sse; do
    if [[ -s "$TMP/$f" ]]; then
      redact_stream < "$TMP/$f" > "$dest/$f" 2>/dev/null || true
      chmod 600 "$dest/$f" 2>/dev/null || true
    fi
  done
  if [[ -n "${LOG:-}" && -s "${LOG:-}" ]]; then
    redact_stream < "$LOG" > "$dest/serve.log" 2>/dev/null || true
    chmod 600 "$dest/serve.log" 2>/dev/null || true
  fi
  printf '%s\n' "$HARNESS_REV" > "$dest/HARNESS_REV"
  chmod 600 "$dest/HARNESS_REV" 2>/dev/null || true
  echo "  sanitized diagnostics saved: $dest" >&2
  set -e
}
...
echo '[REAL 1/14] validate approval + API key + FX + hard limits'
...
if wait < 30 or wait > 120:
    raise SystemExit('BALANCE_V20_REAL_FAIL: wait must be 30..120 seconds')
...
echo '[REAL 2/14] verify installed strict hard-budget one-paid-attempt policy'
grep -q 'strict-one-paid-attempt-v0.20-fixed9' internal/agent/run_budget.go || fail 'Fixed9 core marker missing; refusing paid API call'
! grep -q 'strictPreCallRetryFactor' internal/agent/run_budget.go || fail 'obsolete 66-way retry reserve is still installed'
PATH="$(dirname "$(command -v "$GO_BIN")"):$PATH" GOTOOLCHAIN=local \
  "$GO_BIN" test ./internal/provider \
    -run '^TestBalanceStrictRetryLimitZeroStartsOneHTTPAttempt$' -count=1
PATH="$(dirname "$(command -v "$GO_BIN")"):$PATH" GOTOOLCHAIN=local \
  "$GO_BIN" test ./internal/agent \
    -run '^(TestBalanceStrictPreCallUsesCurrentAttemptEnvelope|TestBalanceStrictReasoningFallbackSuppressed)$' -count=1
echo "  core marker=Fixed9 | harness=${HARNESS_REV} | obsolete 66-way reserve=ABSENT | provider retries=0 | body replay=off | reasoning replay=off"

echo '[REAL 3/14] build current Reasonix CLI'
mkdir -p bin
...
echo '[REAL 4/14] create isolated Reasonix runtime + credential store'
TMP="$(mktemp -d)"
PID=''
SERVER_PID=''
SSE_PID=''
...
echo '[REAL 5/14] zero-generation DeepSeek API preflight (/models + /user/balance)'
set +e
...
echo '[REAL 6/14] verify strict pre-call pricing is known BEFORE Provider.Stream'
"$PY_BIN" - "$TMP/reasonix.toml" "$MANIFEST" <<'PY'
import json,sys,tomllib
cfg=tomllib.load(open(sys.argv[1],'rb'))
m=json.load(open(sys.argv[2],encoding='utf-8'))
ps=[x for x in cfg.get('providers',[]) if x.get('name')=='deepseek-v20']
if len(ps)!=1:
    raise SystemExit('BALANCE_V20_REAL_FAIL: deepseek-v20 provider config missing/duplicated')
p=ps[0]; price=p.get('price')
if not isinstance(price,dict):
    raise SystemExit('BALANCE_V20_REAL_FAIL: provider price missing; v0.16 strict pre-call would fail closed before Provider.Stream')
want=m['pricingSnapshot']['deepseek-v4-flash']
for key,source in [('cache_hit','cacheHit'),('input','input'),('output','output')]:
    got=float(price.get(key,-1)); exp=float(want[source])
    if abs(got-exp)>1e-12:
        raise SystemExit(f'BALANCE_V20_REAL_FAIL: conservative provider price mismatch {key}: got={got} want={exp}')
if price.get('currency')!='USD':
    raise SystemExit('BALANCE_V20_REAL_FAIL: provider price currency must be USD')
print(f"  pricing=KNOWN | official flat hit={price['cache_hit']}/M input={price['input']}/M output={price['output']}/M USD")
PY

echo '[REAL 7/14] verify Reasonix sees the same provider credential'
set +e
(
  cd "$TMP"
...
echo '[REAL 8/14] start token-authenticated localhost backend'
(
  cd "$TMP"
  HOME="$TMP/home" REASONIX_HOME="$TMP/rxhome" \
...
echo '[REAL 9/14] apply and verify hard pre-call KZT budget'
...
echo '[REAL 10/14] open authenticated SSE telemetry'
set +e
...
echo '[REAL 11/14] submit EXACTLY ONE DeepSeek V4 Flash model task'
...
echo '[REAL 12/14] wait for clean provider completion + positive ledger spend'
PASS=0
LAST_SPENT='0'
...
for sec in $(seq 1 "$WAIT_SECONDS"); do
  ...
  TURN_STATE="$("$PY_BIN" scripts/balance_mod_v020_completion_check.py "$TMP/live.json" 2>/dev/null || true)"
  case "$TURN_STATE" in
    ERROR$'\t'*) fail "live terminal event: ${TURN_STATE#*$'\t'}" ;;
  esac

  NEW_LOG="$(tail -n "+$((LOG_SCAN_FROM + 1))" "$LOG" 2>/dev/null || true)"
  if printf '%s\n' "$NEW_LOG" | grep -Eqi \
    '(^|[ =])(ERROR|FATAL)([ =:]|$)|provider.*(error|fail)|deepseek.*(401|402|422|429|500|503)|unauthor|insufficient balance|deadline exceeded|no such host|connection refused|tls.*error|pre-call cost budget.*(cannot|fail|block|deny)|cannot be enforced without model pricing|cannot safely admit prompt'; then
...
PY
  ...
  MARKER_VIS='masked-or-omitted'
  if { [[ -f "$TMP/live.json" ]] && grep -q 'BALANCE_V20_REAL_PROVIDER_OK' "$TMP/live.json"; } || \
     { [[ -f "$TMP/events.sse" ]] && grep -q 'BALANCE_V20_REAL_PROVIDER_OK' "$TMP/events.sse"; }; then
    MARKER_VIS='visible'
  fi

  if [[ "$TURN_STATE" == DONE$'\t'* && "$POSITIVE" == 1 ]]; then
    PASS=1
    VISIBILITY="${TURN_STATE#*$'\t'}"
    echo "  clean provider completion observed in ~${sec}s | spentKzt=${LAST_SPENT} | liveText=${VISIBILITY} | marker=${MARKER_VIS}"
    break
  fi

  if (( sec % 5 == 0 )); then
    echo "  waiting... ${sec}s | spentKzt=${LAST_SPENT} | state=${TURN_STATE:-unknown}"
  fi
  sleep 1
done
[[ "$PASS" == 1 ]] || fail "clean provider completion + positive spend not observed within ${WAIT_SECONDS}s"

echo '[REAL 13/14] verify Flash-only path + reconcile provider-reported usage'
"$CURL_BIN" -fsS --connect-timeout 5 --max-time 10 \
...
[[ -s "$TMP/provider-usage.json" ]] || fail 'exact provider usage receipt not written by emitTurnUsage'
"$PY_BIN" scripts/balance_mod_v020_reconcile.py \
  --manifest "$MANIFEST" \
  --before "$TMP/before.json" \
  --after "$TMP/after.json" \
  --fx "$FX" \
  --usage-receipt "$TMP/provider-usage.json"

echo '[REAL 14/14] final hard-budget assertion'
