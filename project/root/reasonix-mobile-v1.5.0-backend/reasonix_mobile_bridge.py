...
BRIDGE_VERSION = 'reasonix-mobile-v1.5.0'
...
class MobileCore:
    ...
    def integration_audit(self):
        ...
        core_tools=[{'name':n,'available':n in raw,'builtin':True} for n in core_names]
        # Read-only source audit of the currently selected Reasonix tree. This is
        # deliberately diagnostic only: absence is NOT repaired automatically.
        ...
        core_guard={'sourcePresent':bool(guard_src),
          ...
          'path':str(guard_path)}
        reasoning_transport={'eventReasoning':('Reasoning' in event_src and 'ReasoningContent' in event_src),
          'serveHistoryReasoning':('ReasoningContent' in serve_src and 'json:"reasoning' in serve_src),
          'eventPath':str(event_path),'servePath':str(serve_path)}
        return {'ok':sc==200 and cc==200,'bridge':BRIDGE_VERSION,'upstream':self.upstream,'statusHttp':sc,'contractHttp':cc,
          ...
          'coreGuard':core_guard,'reasoningTransport':reasoning_transport,
          'warnings':([ 'Execution Router is disabled/unconfigured; APK Auto cannot honestly claim Flash/Pro auto-routing.' ] if not ex.get('enabled') else [])}
    BUILTIN_SKILLS={
...
def make_handler(core:MobileCore):
    class H(BaseHTTPRequestHandler):
        server_version='ReasonixMobileBridge/1.5.0'
        ...
        def handle_api(self,method):
            ...
            try:
                ...
                if u.path=='/mobile/info' and method=='GET':
                    self.send_json(200,{'model':core.model,'upstream':core.upstream,'root':str(core.root),'skillsRoot':str(core.skills_root),'androidTools':str(core.android_tools_script),'bridgeVersion':'1.5.0'});return
                n=int(self.headers.get('Content-Length','0') or 0)
