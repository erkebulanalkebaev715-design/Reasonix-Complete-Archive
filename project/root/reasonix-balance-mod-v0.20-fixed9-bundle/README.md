Reasonix Balance Mod v0.20 Fixed9
...
## Fixed9 installer hardening

Fixed9 fixes the local compile failure where an older generated
...
files from `internal/agent` and `internal/provider`, keeps only Fixed9 tests, and
...
Fixed9 installer hardening: targeted changed-behavior tests are executed; touched
packages are then compile-checked without running unrelated Android/PRoot-sensitive
session stress tests. A full CLI build remains mandatory before installation commits.
