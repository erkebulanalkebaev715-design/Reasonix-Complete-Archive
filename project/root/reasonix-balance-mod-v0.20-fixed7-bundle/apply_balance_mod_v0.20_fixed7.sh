...
grep -q 'const balanceModVersion = "balance-mod-v0.20"' internal/serve/mod_bridge.go || { echo 'ERROR: Fixed7 requires installed balance-mod-v0.20' >&2; exit 3; }
...
PATHS=(
  ...
  docs/BALANCE_MOD_V020_FIXED7.md
  ...
  internal/agent/balance_strict_retry_fixed7_test.go
  internal/provider/balance_strict_retry_fixed7_test.go
)
for p in "${PATHS[@]}"; do [[ -f "$PAYLOAD/$p" ]] || { echo "ERROR: payload missing $p" >&2; exit 2; }; done

TMP="$(mktemp -d)"
PATCH="$TMP/v020-fixed7.patch"
BACKUP="$TARGET/.balance_mod_backups/v0.20-fixed7-$(date +%Y%m%d-%H%M%S)"
mkdir -p "$BACKUP"
APPLIED=0
VALIDATED=0
...
rollback_on_error() {
  ...
  if [[ $rc -ne 0 && $APPLIED -eq 1 && $VALIDATED -eq 0 ]]; then
    echo "Fixed7 local validation failed; restoring pre-install files..." >&2
    for p in "${CORE_PATHS[@]}" "${PATHS[@]}"; do
      if [[ -e "$BACKUP/$p" ]]; then
...
new="""// strict-one-paid-attempt-v0.20-fixed7: strict hard-budget mode allows exactly
...
elif 'strict-one-paid-attempt-v0.20-fixed5' in s or 'strict-one-paid-attempt-v0.20-fixed6' in s:
    s=s.replace('strict-one-paid-attempt-v0.20-fixed5','strict-one-paid-attempt-v0.20-fixed7')
    s=s.replace('strict-one-paid-attempt-v0.20-fixed6','strict-one-paid-attempt-v0.20-fixed7')
elif 'strict-one-paid-attempt-v0.20-fixed7' not in s:
...
if 'strictPreCallRetryFactor' in s: raise SystemExit('ERROR: obsolete strictPreCallRetryFactor survived Fixed7')
if 'strict-one-paid-attempt-v0.20-fixed7' not in s: raise SystemExit('ERROR: Fixed7 run_budget marker missing')
p.write_text(s)
...
\t\t// strict-one-paid-attempt-v0.20-fixed7: no hidden connection/header retry.
...
if old in s: s=s.replace(old,new,1)
    ...
    s=s.replace('strict-one-paid-attempt-v0.20-fixed5','strict-one-paid-attempt-v0.20-fixed7').replace('strict-one-paid-attempt-v0.20-fixed6','strict-one-paid-attempt-v0.20-fixed7')
...
if 'strict-one-paid-attempt-v0.20-fixed7' not in s:
    if 'strict-one-paid-attempt-v0.20-fixed5' in s or 'strict-one-paid-attempt-v0.20-fixed6' in s:
        s=s.replace('strict-one-paid-attempt-v0.20-fixed5','strict-one-paid-attempt-v0.20-fixed7').replace('strict-one-paid-attempt-v0.20-fixed6','strict-one-paid-attempt-v0.20-fixed7')
    else:
        ...
        repl=') (streamedTurn, bool) {\n\tif a.StrictPreCallBudget() {\n\t\t// strict-one-paid-attempt-v0.20-fixed7: fallback would be a second paid call.\n\t\treturn streamedTurn{}, false\n\t}\n\tif !a.activateMissingReasoningFallback() {'
...
if 'strict-one-paid-attempt-v0.20-fixed7' not in s:
    if 'strict-one-paid-attempt-v0.20-fixed5' in s or 'strict-one-paid-attempt-v0.20-fixed6' in s:
        ...
        injected=m.group(1)+'''\t\tif a.StrictPreCallBudget() {\n\t\t\t// strict-one-paid-attempt-v0.20-fixed7: replay is another paid request.\n\t\t\tshouldRetry = false\n\t\t}\n'''+m.group(2)
...
if [[ -s "$PATCH" ]]; then
  git apply --check "$PATCH" || { echo 'ERROR: Fixed7 exact apply check failed; refusing fuzzy apply' >&2; exit 4; }
  echo 'v0.20 Fixed7 exact apply check: PASS'
  git apply "$PATCH"
  APPLIED=1
else
  echo 'v0.20 Fixed7 already exact: PASS'
fi

"$GOFMT_BIN" -w "${CORE_PATHS[@]}" internal/agent/balance_strict_retry_fixed7_test.go internal/provider/balance_strict_retry_fixed7_test.go
...
echo 'v0.20 Fixed7 targeted tests...'
...
echo 'v0.20 Fixed7 package compile/regression...'
PATH="$(dirname "$GO_BIN"):$PATH" GOTOOLCHAIN=local "$GO_BIN" test ./internal/provider ./internal/agent ./internal/serve
PATH="$(dirname "$GO_BIN"):$PATH" GOTOOLCHAIN=local CGO_ENABLED=0 "$GO_BIN" build -o "$TMP/reasonix-fixed7" ./cmd/reasonix
[[ -x "$TMP/reasonix-fixed7" ]] || { echo 'ERROR: Fixed7 temporary CLI build missing' >&2; exit 5; }

grep -q 'strict-one-paid-attempt-v0.20-fixed7' internal/agent/run_budget.go || { echo 'ERROR: Fixed7 marker missing after validation' >&2; exit 5; }
...
mkdir -p bin
cp "$TMP/reasonix-fixed7" bin/reasonix
chmod +x bin/reasonix
VALIDATED=1

echo 'v0.20 Fixed7 local validation: PASS'
