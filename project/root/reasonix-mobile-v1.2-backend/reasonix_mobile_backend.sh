...
cmd_api(){
  ...
  python3 - "$path" "$method" "$TOKEN" <<'PYAPI'
import json,sys,urllib.request,urllib.error
path,method,tf=sys.argv[1:4]
token=open(tf,encoding='utf-8').read().strip()
req=urllib.request.Request('http://127.0.0.1:37914'+path,data=(b'{}' if method=='POST' else None),method=method,headers={'X-Reasonix-Mobile-Token':token,'Content-Type':'application/json'})
try:
...
try:
 print(urllib.request.urlopen('http://127.0.0.1:37914/mobile/health',timeout=5).read().decode())
