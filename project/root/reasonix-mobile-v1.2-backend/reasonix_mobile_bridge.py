...
class MobileCore:
    ...
    def diagnostics(self):
        ...
        try:
            code,_,body=self._request_upstream('GET','/mod/status',b'',None,timeout=8)
        ...
        return {'ok':bool(self.proc and self.proc.poll() is None and upstream_status and upstream_status.get('http')==200),'bridge':'reasonix-mobile-v1.2','model':self.model,'root':str(self.root),'upstream':self.upstream,'upstreamStatus':upstream_status,'upstreamError':upstream_error,'doctor':doctor_summary,'android':self.android_capabilities(),'reasonixLogTail':self._tail(self.up_log,8000)}



def make_handler(core:MobileCore):
    class H(BaseHTTPRequestHandler):
        server_version='ReasonixMobileBridge/1.2'
        ...
        def handle_api(self,method):
            ...
            if u.path=='/mobile/health':self.send_json(200,{'ok':True,'bridge':'reasonix-mobile-v1.2','model':core.model,'tokenRequired':True});return
            ...
            try:
                ...
                if u.path=='/mobile/info' and method=='GET':
                    self.send_json(200,{'model':core.model,'upstream':core.upstream,'root':str(core.root),'skillsRoot':str(core.skills_root),'androidTools':str(core.android_tools_script),'bridgeVersion':'1.2'});return
                n=int(self.headers.get('Content-Length','0') or 0)
