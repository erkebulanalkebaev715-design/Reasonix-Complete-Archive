...
def android_su():
    # Never treat Debian/PRoot /usr/bin/su as Android root. Only Android/Termux
    # host-side candidates are eligible; actual invocation may still fail if the
    # phone is not rooted or permission is denied.
    candidates=[
      TERMUX_PREFIX/'su', pathlib.Path('/system/bin/su'), pathlib.Path('/system/xbin/su'),
      pathlib.Path('/sbin/su'), pathlib.Path('/su/bin/su')]
    for p in candidates:
        try:
...
def adb_status():
    # Truthful ADB availability: the binary existing is not a device being
    # connected, and a connection is not authorization. All three are reported
    # separately and never conflated.
    adb=which('adb')
    out={'binary':bool(adb),'path':adb,'devices':[],'connected':False,'authorized':False}
    if not adb:return out
    r=run([adb,'devices'])
    if r['exitCode']!=0:return out
    for ln in r['stdout'].splitlines():
        ln=ln.strip()
        if not ln or ln.startswith('*') or 'daemon' in ln or 'List of devices' in ln:continue
        parts=ln.split()
        if len(parts)>=2:
            state=parts[1].lower();out['devices'].append({'serial':parts[0],'state':state})
            if state=='device':
                out['connected']=True;out['authorized']=True
            elif state in ('unauthorized','offline'):
                out['connected']=True
    return out

def caps():
    termux={n:which(n) for n in TERMUX_NAMES};rish=which('rish');su=android_su();system_sh='/system/bin/sh' if pathlib.Path('/system/bin/sh').exists() else ''
    return {'termuxApi':bool(any(termux.values())),'termuxTools':sorted(k for k,v in termux.items() if v),
      ...
      'note':'Path visibility is not privilege. rootSu excludes Debian /usr/bin/su; /system/bin/sh still runs in the app/PRoot security context unless Android itself grants more.'}
...
def handle(req):
    ...
    if m=='initialize':result={'protocolVersion':PROTO,'capabilities':{'tools':{}},'serverInfo':{'name':'reasonix-android-host','version':'1.3.2'}}
