...
cmd_full_stack_audit(){
  echo '=== environment ==='
  echo "reasonix_root=$ROOT"
  echo "reasonix_binary=$BIN"
  echo "path=$PATH"
  echo "gotoolchain=${GOTOOLCHAIN:-<unset>}"
  printf 'go_path='; command -v go 2>/dev/null || echo '<missing>'
  printf 'go_default='; go version 2>/dev/null || echo '<unavailable>'
  if [[ -x /usr/local/go/bin/go ]]; then printf 'go_usr_local='; /usr/local/go/bin/go version; else echo 'go_usr_local=<missing>'; fi
  echo "root_exists=$([[ -d "$ROOT" ]] && echo true || echo false)"
  echo "binary_exists=$([[ -x "$BIN" ]] && echo true || echo false)"
  if [[ -d "$ROOT/.git" ]]; then
    printf 'git_head='; git -C "$ROOT" rev-parse HEAD 2>/dev/null || echo '<unknown>'
    printf 'git_dirty_files='; git -C "$ROOT" status --porcelain 2>/dev/null | wc -l | tr -d ' '
  else
    echo 'git_head=<not-a-git-tree>'
  fi
  echo '=== backend ==='
  cmd_status
  echo '=== integration ==='
  cmd_integration_audit
}
cmd_port_debug(){
