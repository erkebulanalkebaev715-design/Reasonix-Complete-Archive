...
class MobileCore:
    ...
    # /rewind is the checkpoint-based session rewind used by the APK's Retry
    # action ("rerun this user turn, no duplicate history"). It is a mutation,
    # so it stays POST-only like the other CONTROL_ROUTES.
    CONTROL_ROUTES={'/approve','/answer','/tool-approval-mode','/plan','/rewind'}
    ...
    def set_system_prompt(self,obj):
        ...
        if m:
            ...
            if re.search(r'(?m)^[ \t]*system_prompt[ \t]*=',block):
                # re.sub replacement strings re-parse backslash escapes; a
                # prompt containing "\n" (every multiline prompt) was being
                # written as raw newlines, breaking the TOML single-line value.
                # A callable replacement inserts the JSON string verbatim.
                if prompt:block=re.sub(r'(?m)^[ \t]*system_prompt[ \t]*=.*$',lambda _m:f'system_prompt = {encoded}',block,count=1)
