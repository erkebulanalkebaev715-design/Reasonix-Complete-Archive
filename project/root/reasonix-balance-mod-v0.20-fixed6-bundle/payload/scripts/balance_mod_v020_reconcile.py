...
def candidates(roots):
    ...
    for root in roots:
        for n in walk(root):
            ...
            miss=max(0,p-hit) if miss is None else miss
            model=""
            for k,v in n.items():
...
def calc(u,m,fx):
    r=m["pricingSnapshot"]["deepseek-v4-flash"]
    unit=m["pricingSnapshot"]["unitTokens"]
...
def selftest():
    ...
    usd,kzt=calc(u,m,500)
    assert usd>0 and kzt>0
...
if abs(delta-kzt)>tol:
    raise SystemExit(f"V020_RECONCILE_FAIL: expectedKzt={kzt:.6f} backendDeltaKzt={delta:.6f} tolerance={tol:.6f} usage={u}")
