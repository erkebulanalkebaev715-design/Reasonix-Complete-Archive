...
WAIT_SECONDS="${BALANCE_V20_WAIT_SECONDS:-60}"
...
echo '[REAL 1/15] validate approval + API key + FX + hard limits'
...
if wait < 30 or wait > 90:
    raise SystemExit('BALANCE_V20_REAL_FAIL: wait must be 30..90 seconds')
...
echo '[REAL 2/15] verify strict hard-budget one-paid-attempt policy'
...
PATH="$(dirname "$(command -v "$GO_BIN")"):$PATH" GOTOOLCHAIN=local \
  "$GO_BIN" test ./internal/provider/openai \
    -run '^TestBalanceStrictSingleAttempt(UsesNonStreamingDeepSeek|HasHardProviderTimeout)$' -count=1
echo '  strict transport=NONSTREAM | provider deadline=45s | hidden retries=0' 

echo '[REAL 3/15] build current Reasonix CLI'
mkdir -p bin
...
echo '[REAL 4/15] create isolated Reasonix runtime + credential store'
TMP="$(mktemp -d)"
PID=''
SERVER_PID=''
SSE_PID=''
...
echo '[REAL 5/15] zero-generation DeepSeek API preflight (/models + /user/balance)'
set +e
...
echo '[REAL 6/15] verify strict pre-call pricing is known BEFORE Provider.Stream'
...
for key,source in [('cache_hit','cacheHit'),('input','input'),('output','output')]:
    ...
    if abs(got-exp)>1e-12:
        raise SystemExit(f'BALANCE_V20_REAL_FAIL: provider price mismatch {key}: got={got} want={exp}')
...
echo '[REAL 7/15] verify Reasonix sees the same provider credential'
set +e
(
  cd "$TMP"
...
echo '[REAL 8/15] strict DeepSeek transport policy
  one HTTP request | stream=false | thinking=disabled | timeout=45s | tools=0'

echo '[REAL 9/15] start token-authenticated localhost backend'
(
  cd "$TMP"
  HOME="$TMP/home" REASONIX_HOME="$TMP/rxhome" \
...
echo '[REAL 10/15] apply and verify hard pre-call KZT budget'
...
echo '[REAL 11/15] open authenticated SSE telemetry'
set +e
...
echo '[REAL 12/15] submit EXACTLY ONE DeepSeek V4 Flash model task'
...
echo '[REAL 13/15] wait for provider result; fail immediately on typed/runtime error'
PASS=0
LAST_SPENT='0'
...
echo '[REAL 14/15] verify Flash-only path + reconcile provider-reported usage'
"$CURL_BIN" -fsS --connect-timeout 5 --max-time 10 \
...
echo '[REAL 15/15] final hard-budget assertion'
