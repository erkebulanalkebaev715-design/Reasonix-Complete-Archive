...
case "${1:-status}" in start)cmd_start;; stop)cmd_stop;; restart)cmd_stop;cmd_start;; status)cmd_status;; token)cmd_token;; log)cmd_log;; diagnose)cmd_diagnose;; port-debug)cmd_port_debug;; install-android-tools)cmd_android_tools;; *)echo "usage: $0 start|stop|restart|status|token|log|diagnose|port-debug|install-android-tools";exit 2;; esac
